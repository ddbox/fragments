...
function is_vm_up() {
    ...
    while [ $retries -lt 30 ] ; do
        tmpout=`timeout 5 ssh  -o StrictHostKeyChecking=no $fqdn hostname 2>&1`
        if [ "$tmpout" != "$fqdn" ] ; then
            echo -n "."
            sleep 30
...
add_dn_to_condor_mapfile "/DC=org/DC=opensciencegrid/O=Open Science Grid/OU=Services/CN=fermicloud046.fnal.gov" ce1
add_dn_to_condor_mapfile "/DC=org/DC=opensciencegrid/O=Open Science Grid/OU=Services/CN=fermicloud105.fnal.gov" ce2
add_dn_to_condor_mapfile "/DC=org/DC=opensciencegrid/O=Open Science Grid/OU=Services/CN=fermicloud127.fnal.gov" ce3
add_dn_to_condor_mapfile "/DC=org/DC=opensciencegrid/O=Open Science Grid/OU=Services/CN=fermicloud378.fnal.gov" ce4
...
fact_vm_dn="/DC=org/DC=opensciencegrid/O=Open Science Grid/OU=Services/CN=$fact_fqdn"
vofe_vm_dn="/DC=org/DC=opensciencegrid/O=Open Science Grid/OU=Services/CN=$vofe_fqdn"
r
...
if [ -n "$condor_tarball" ]; then
    timeout 5 ssh root@$fact_fqdn 'mkdir -p /var/lib/gwms-factory/condor/' 
    if [ -f "$condor_tarball" ]; then
        ...
        timeout 90 ssh root@$fact_fqdn "$cmd"
        #if [ $? -ne 0 ]; then
...
if [ -n "$condor_tarball2" ]; then
    timeout 4 ssh root@$fact_fqdn 'mkdir -p /var/lib/gwms-factory/condor/' 
...
echo "timeout 600 ssh root@$fact_fqdn /tmp/fact_install.sh " > /tmp/ssh_fact.$TS.sh
...
grep 'FACTORY VERIFICATION: SUCCESS'  $FACT_LOG.$TS  > /dev/null 2>&1  || { 
    echo "================================================================================"
...
echo "timeout 600 ssh root@$vofe_fqdn /tmp/vofe_install.sh" > /tmp/ssh_frontend.$TS.sh
